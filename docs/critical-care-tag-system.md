...
## v1.4 — hand-authored mapping is now the source of truth
The tags are no longer generated by a classifier. The authoritative mapping is the
hand-curated **`tools/critical_care_tags_manual.tsv`** (every note tagged by reading its
content); `tools/build_tags.py` validates it against the collection (completeness +
controlled vocabulary) and emits the JSON + `critical-care-tag-mapping.tsv`. To retag,
edit the manual TSV. Hand adjustments made during the full read that the rules had missed:
- **ECG electrode-placement** cards → `management` (a hands-on procedure, like art-line
  zeroing / CVC placement), not `diagnostics`.
- **PaO₂ / PaCO₂ intubation thresholds** → `diagnostics` (numeric criteria, consistent with
  the GCS / RR / pH intubation-criteria cards), not `management`.
- **Cross-system:** "excessive post-op bleeding" → `+hematologic`; massive-transfusion drug
  table → `+perioperative`. (14 notes now span >1 system.)
